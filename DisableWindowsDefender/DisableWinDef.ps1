...
Write-Host "[+] Disable Windows Defender (as $(whoami))"  # Write-Host cmdlet is used to write the customized output to a host. 
...
67..90|foreach-object{    #iterating from all posible ascii code for drive path and add
    $drive = [char]$_
...
Remove-Item -Force C:\ProgramData\Microsoft\Windows\Start Menu\Programs\StartUp\disable-defender.lnk -ErrorAction 'ignore'
Write-Host "    [+] StartUp Link Deleted" -BackgroundColor white -ForegroundColor black
